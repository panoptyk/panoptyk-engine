...
export class Faction extends IDObject {
   ...
   */
  constructor(name: string, type: string, headquarters?: Room, id?: number) {
    super(Faction.name, id);
    this._factionName = name;
    this._factionType = type;
    this._headquarters = headquarters ? headquarters.id : 1;
  ...
  static load(json: Faction) {
    ...
    f = f
      ? f
      : new Faction(json._factionName, json._factionType, undefined, json.id);
