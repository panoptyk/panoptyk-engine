...
describe("TradeManipulator", () => {
    ...
    context("Add Trade to Active Trades", () => {
        it("Once", () => {
            db.storeModel(trade);

            TradeManipulator.addToActiveTrades(trade);

            assert.deepEqual(new Set<number>([1]), Trade.activeTrades);
        });
        it("Multiple Times", () => {
            const trade2 = new Trade(initiator, receiver, conversation);
            const trade3 = new Trade(initiator, receiver, conversation);
            const trade4 = new Trade(initiator, receiver, conversation);
            
            db.storeModels([trade, trade2, trade3, trade4]);

            TradeManipulator.addToActiveTrades(trade2);
            TradeManipulator.addToActiveTrades(trade3);
            TradeManipulator.addToActiveTrades(trade4);

            assert.deepEqual(new Set([1, 2, 3, 4]), Trade.activeTrades);
        });
    });
    context("Remove Trade From Active Trades", () => {
        it("Once When Multiple Are Present", () => {
            ...
            TradeManipulator.removeFromActiveTrades(trade2);

            assert.sameDeepMembers([1, 3, 4], Array.from(Trade.activeTrades));
        });
        it("Multiple Times", () => {
            const trade2 = new Trade(initiator, receiver, conversation);
            const trade3 = new Trade(initiator, receiver, conversation);
            ...
            assert.deepEqual(new Set([1, 3, 4]), Trade.activeTrades);

            TradeManipulator.removeFromActiveTrades(trade);
            TradeManipulator.removeFromActiveTrades(trade4);
            
            assert.sameDeepMembers([3], Array.from(Trade.activeTrades));
        });
        it("Once", () => {
            const trade2 = new Trade(initiator, receiver, conversation);
            const trade3 = new Trade(initiator, receiver, conversation);
            ...
            TradeManipulator.removeFromActiveTrades(trade3);

            assert.equal(0, Trade.activeTrades.size);
        }); 
    });
    context("Add Items", () => {
        it("Once", () => {
